...
export class CommunityService {
    ...
    async create(createCommunityDto: CreateCommunityDto, requestUser: any) {
        ...
        if (requestUser.id != createCommunityDto.admin.id) {
            throw new BadRequestException(`Attempt to create a community with user ${createCommunityDto.admin.id} as admin while logged with a different user ${requestUser.id}`);
        }
    ...
    async update(id: number, updateCommunityDto: UpdateCommunityDto, requestUser: any) {
        this.findOne(id);
    ...
    async addUser(id: number, userReferenceDto: UserReferenceDto, requestUser: any) {
        await this.validateUserIsAdminFromCommunity(requestUser.id, id);

        let user = await this.fetchUser(userReferenceDto);

        if (user.community != undefined) {
            throw new BadRequestException(`User ${user.id} takes part in a community already!`);
        }
    ...
    async removeUser(id: number, userReferenceDto: UserReferenceDto, requestUser: any) {
        await this.validateUserIsAdminFromCommunity(requestUser.id, id)

        let user = await this.fetchUser(userReferenceDto);

        if (user.community == undefined || (user.community.id != id)) {
            throw new BadRequestException(`User ${user.id} does not belong to the community ${id}`);
        }
    ...
    async fetchUser(userReferenceDto: UserReferenceDto) {
        ...
        if (userReferenceDto.id) {
            user = await userRepository.findOne({ id: userReferenceDto.id }, { relations: ["community"] });
        }
        else if (userReferenceDto.email) {
            user = await userRepository.findOne({ email: userReferenceDto.email }, { relations: ["community"] });
        }
        else if (userReferenceDto.nickname) {
            user = await userRepository.findOne({ nickname: userReferenceDto.nickname }, { relations: ["community"] });
        }
        else {
            throw new NotFoundException(`No user was found with attributes id: ${userReferenceDto.id}, email: ${userReferenceDto.email}, nickname: ${userReferenceDto.nickname}`);
        }
        return user;
    }
    ...
    async validateUserIsAdminFromCommunity(userId: number, communityId: number) {
        ...
        if (!userIsAdmin) {
            throw new UnauthorizedException(`User ${userId} is not admin from community ${communityId}`);
        }
    }
